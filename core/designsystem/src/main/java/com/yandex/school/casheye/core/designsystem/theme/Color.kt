package com.yandex.school.casheye.core.designsystem.theme

import androidx.compose.ui.graphics.Color

val BrandPrimary = Color(0xFF6750A4)
val BrandPrimaryDark = Color(0xFFD0BCFF)
val BrandOnPrimary = Color.White
val BrandSecondary = Color(0xFF625B71)
val BrandSecondaryDark = Color(0xFFCCC2DC)
val BrandOnSecondary = Color.White

val BrandBackground = Color(0xFFFEF7FF)
val BrandBackgroundDark = Color(0xFF141218)
val SurfacePrimary = Color(0xFFFEF7FF)
val SurfacePrimaryDark = Color(0xFF141218)
val SurfaceVariant = Color(0xFFF3EDF7)
val SurfaceVariantDark = Color(0xFF49454F)
val SurfaceDim = Color(0xFFDED8E1)
val SurfaceDimDark = Color(0xFF141218)
val SurfaceBright = Color(0xFFFEF7FF)
val SurfaceBrightDark = Color(0xFF3B383E)
val SurfaceContainerLowest = Color.White
val SurfaceContainerLowestDark = Color(0xFF0F0D13)
val SurfaceContainerLow = Color(0xFFF7F2FA)
val SurfaceContainerLowDark = Color(0xFF1D1B20)
val SurfaceContainer = Color(0xFFF3EDF7)
val SurfaceContainerDark = Color(0xFF211F26)
val SurfaceContainerHigh = Color(0xFFECE6F0)
val SurfaceContainerHighDark = Color(0xFF2B2930)
val SurfaceContainerHighest = Color(0xFFE6E0E9)
val SurfaceContainerHighestDark = Color(0xFF36343B)

val TextPrimary = Color(0xFF1D1B20)
val TextPrimaryDark = Color(0xFFE6E0E9)
val TextSecondary = Color(0xFF49454F)
val TextSecondaryDark = Color(0xFFCAC4D0)

val BorderDefault = Color(0xFF79747E)
val BorderDefaultDark = Color(0xFF938F99)
val BorderVariant = Color(0xFFCAC4D0)
val BorderVariantDark = Color(0xFF49454F)
val Error = Color(0xFFB3261E)
val ErrorDark = Color(0xFFF2B8B5)
val OnError = Color.White
val OnErrorDark = Color(0xFF601410)
val ErrorContainer = Color(0xFFF9DEDC)
val ErrorContainerDark = Color(0xFF8C1D18)
val OnErrorContainer = Color(0xFF410E0B)
val OnErrorContainerDark = Color(0xFFF9DEDC)
val InverseSurface = Color(0xFF322F35)
val InverseSurfaceDark = Color(0xFFE6E0E9)
val InverseOnSurface = Color(0xFFF5EFF7)
val InverseOnSurfaceDark = Color(0xFF322F35)

val FabContainer = Color(0xFFEADDFF)
val ColorOnPrimaryContainer = Color(0xFF21005D)
val NavigationSelected = Color(0xFFE8DEF8)
val ColorOnSecondaryContainer = Color(0xFF1D192B)

val ChartExpense = Color(0xFFC43E3E)
val ChartExpenseDark = Color(0xFFFF8585)
val ChartIncome = Color(0xFF329E5D)
val ChartIncomeDark = Color(0xFF72DB9C)
val ChartOther = Color(0xFFC65A77)
val ChartOtherDark = Color(0xFFFF9EB8)
